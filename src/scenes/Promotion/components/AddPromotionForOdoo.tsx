...
import { Button, Card, DatePicker, Drawer, Form, Input, Layout, Space, Typography } from 'antd';
...
import DealsCollectionsForOdoo from './DealsCollectionsForOdoo';
import RewardSelectForOdoo from './RewardSelectForOdoo';
...
const AddPromotion: React.FC<IPromotionStore> = ({ ...props }) => {
  ...
  const HandelSave = async () => {
    ...
    setOnloading(true);
         ...
         converValue.deals.map((item: any) => {
           return {
             ...
             redeem: {
               ...
               quantity: item.rewardType == AppConsts.RewardTypeGIFT
     ...
     setTimeout(async()=>{
      ...
      setOpen(false);
      form.resetFields();
      promotion?.GetAllList({ keyword: '', maxResultCount: 100, skipCount: 0 });  
  }, 3000);
  ...
  return (
    <>
      ...
      <Drawer
        title="Add New Promotion For Odoo"
        placement="right"
        size={size}
        onClose={onClose}
        open={open}
